...
import {
  ControllerEditorContext,
  IControllerEditorContextParam,
  IEventMeta
} from '@rxdrag/minions-controller-editor';
...
export const ControllerMetaEditorAntd5 = memo(
  (props: ControllerMetaEditorAntd5Props) => {
    ...
    const logicFlowMeta = useMemo(() => {
      const logicflow = value?.reactions?.find(
    ...
    const handleChange = useCallback(
      (newMetas: ILogicMetas) => {
        const newValue = {
          ...value,
          reactions: value?.reactions?.map(reaction =>
            reaction.id === selected ? { ...reaction, ...newMetas } : reaction
          ),
          events: value?.events?.map(event =>
            event.id === selected ? { ...event, ...newMetas } : event
          )
        };
        onChange?.(newValue);
      },
