...
export const SettingsForm = memo(() => {
    ...
    const propsSchema = useMemo(() => {
        if (currentNode && currentNode.propsSchema) {
            ...
            return engine?.getLoacalesManager()
                .translateDesignerSchema(currentNode?.meta.componentName, JSON.parse(JSON.stringify(currentNode.propsSchema)));
        }
        else {
            return undefined;
        }
    ...
    return (_jsx(PreviewRoot, { components: {
            Tabs,
            TabPanel,
            FormItem: Form.Item,
            Input,
            ...
            Space,
            ...tools,
        }, localesManager: engine?.getLoacalesManager(), children: _jsx(Fieldy, { children: _jsx("div", { style: propertiesStyle, children: currentNode &&
                    _jsx(VirtualForm, { initialValue: currentNode?.meta, onValueChange: handleMetaChange, children: _jsx(Form, { labelAlign: "left", colon: false, labelCol: { span: 7 }, wrapperCol: { span: 17 }, autoComplete: "off", labelWrap: true, style: {
                                flex: 1,
                                height: '100%',
                            }, children: propsSchema &&
                                _jsx(ComponentRender, { root: propsSchema }) }) }, currentNode.id) }) }) }));
});
