...
export const ControllerMetaEditor = memo(props => {
  ...
  const [localesManager] = useState(
    new LocalesManager(lang, minionsEditorLocales)
  );
  useEffect(() => {
  ...
  const metas = useMemo(() => {
    const reaction = value?.reactions?.find(
      reaction => reaction.id === selected
    );
    if (reaction) {
      ...
      const newValue = {
        ...
        reactions: value?.reactions?.map(reaction =>
          reaction.id === selected
            ? { ...reaction, logicMetas: newMetas }
            : reaction
        ),
        events: value?.events?.map(event =>
          event.id === selected ? { ...event, logicMetas: newMetas } : event
        )
      };
      onChange?.(newValue);
    },
  ...
  return _jsx(LocalesContext.Provider, {
    value: localesManager,
    children: _jsx(Minions, {
      materials: materials,
      children: _jsx(ControllersContext.Provider, {
        value: controllerMetas,
        children: _jsx(ControllerContext.Provider, {
          value: value,
          children: _jsxs(SytledContent, {
            id: 'reactions-editor-container',
            children: [
              _jsx(LeftArea, {
                children: _jsx(Members, {
                  value: value,
                  selected: selected,
              ...
              selected &&
                ...
                _jsx(
                  ReactionMetaEditor,
                  { metas: metas, onChange: handleChange, toolbox: toolbox },
                  selected
                )
            ]
          })
