...
export const TooltipDesigner = memo(forwardRef((props, ref) => {
    const { title, children, [RXID_ATTR_NAME]: rxId, ...other } = props;
    ...
    return (_jsx("div", { ref: realRef, style: { display: "inline-block", position: "relative" }, ...open ? {} : { [RXID_ATTR_NAME]: rxId }, onMouseEnter: handleMouseEnter, onMouseLeave: handleMouseLeave, children: _jsxs(Tooltip, { open: open, title: _jsxs(_Fragment, { children: [title, _jsx(CloseButton, { style: {
                            top: -16,
                            right: -16
                        }, onClick: handleClose }), _jsx("div", { ref: handlePopRefChange })] }), ...other, children: [children, (hover || currentNode?.id === node?.id) && !open && _jsx(PopupButton, { onClick: handleShow })] }) }));
}));
